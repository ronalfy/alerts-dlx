...
var setDismissCookie = function setDismissCookie(name, value, maxAge) {
  ...
  var boundedMaxAge = Math.min(31536000, Math.max(1, maxAge));
  var expires = new Date(Date.now() + boundedMaxAge * 1000).toUTCString();
  var cookie = "".concat(name, "=").concat(encodeURIComponent(value));
  cookie += "; Path=/";
  cookie += "; Max-Age=".concat(boundedMaxAge);
...
var bindCloseButtons = function bindCloseButtons() {
  document.querySelectorAll('.alerts-dlx-close').forEach(function (closeButton) {
    ...
    closeButton.addEventListener('click', function () {
      ...
      alert.addEventListener('animationend', removeAlert, {
        once: true
      });
      if ((_window$matchMedia = (_window = window).matchMedia) !== null && _window$matchMedia !== void 0 && _window$matchMedia.call(_window, '(prefers-reduced-motion: reduce)').matches) {
        removeAlert();
      } else {
